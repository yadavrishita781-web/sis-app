...
export function StudentSubjects() {
  ...
  const grouped = subjects.reduce((acc: any, sub: any) => {
    const dept = departments.find((d: any) => d.id === sub.departmentId || d.id === sub.department_id);
    const deptName = dept?.name || sub.departmentName || 'General';
    const key = `${deptName} · Sem ${sub.semester || 1}`;
    acc[key] = acc[key] || [];
    acc[key].push(sub);
    return acc;
  }, {});

  if (loadingSubjects || loadingFaculty) {
    return (
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Enrolled subjects and faculty information ({subjects.length} subjects)</p>
        </div>
      </div>

      {Object.entries(grouped).map(([group, subs]: [string, any]) => (
        <div key={group} className="card">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {subs.map((s: any) => {
              const fac = faculty.find((f: any) => f.id === s.facultyId || f.id === s.faculty_id || f.name === s.facultyName);
              const facultyName = fac?.name || s.facultyName || 'Faculty Member';
              return (
                <div key={s.id} className="p-4 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-300 dark:hover:border-indigo-600 transition-colors">
                  ...
                  <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                    <p className="text-xs text-slate-500">Faculty Instructor</p>
                    <p className="text-sm text-slate-700 dark:text-slate-300 font-medium mt-0.5">{facultyName}</p>
                  </div>
                </div>
              );
            })}
